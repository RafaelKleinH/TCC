...
class MainTabBarCoordinator: CoordinatorProtocol {
    ...
    func start() {
        ...
        let homeTabBarItem = UITabBarItem(title: "homeTabBar".localized(), image: hokeImage?.withTintColor(RFKolors.primaryBlue).resize(targetSize: CGSize(width: 32, height: 32)), tag: 0)
        ...
        let reportTabBarItem = UITabBarItem(title: "reportTabBar".localized(), image: reportImage?.withTintColor(RFKolors.primaryBlue).resize(targetSize: CGSize(width: 32, height: 32)), tag: 1)
        ...
        let healthTabBarItem = UITabBarItem(title: "healthTabBar".localized(), image: healthImage?.withTintColor(RFKolors.primaryBlue).resize(targetSize: CGSize(width: 32, height: 32)), tag: 2)
        ...
        let configTabBarItem = UITabBarItem(title: "configTabBar".localized(), image: UIImage.actions, tag: 3)
        ...
        healthViewModel.navigationTarget
            .subscribe(onNext: { [weak navigationController] target in
                guard let nav = navigationController else { return }
                switch target {
                case let .nextView(select):
                    HealthSecCoordinator(navC: nav, selectedRow: select).start()
                }
            }).disposed(by: healthViewModel.myDisposeBag)
